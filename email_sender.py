...
class EmailSender:
    ...
    def __init__(self, from_email: str = "", email_manager: EmailManager = None, verbose: bool = False):
        ...
        # Try to detect email backend
        if self.sendgrid_key:
            self.backend = "sendgrid"
            if verbose:
                print("[email_sender] Using SendGrid backend")
        else:
            ...
            if smtp_host:
                ...
                self.backend = None
                print("[email_sender] WARNING: No email backend configured (SendGrid or SMTP)")
